...
> [!INFO]- About pseudorandom numbers
...
- Considering `from random import *` which delivers some mechanisms allowing you to operate with **pseudorandom numbers**
	...
	- `seed()` is able to directly **set the generator's seed**. We'll show you two of its variants:
		...
		- IMPORTANT: If the seed is always set with the same value, the sequence of generated values always looks the same. Example: seed is set with `seed(0)` then the results are `0.844421851525`then `0.75795440294`, then `0.420571580831` then `0.258916750293` then `0.511274721369`, etc, always.
