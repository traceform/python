...
<details>
  ...
  - Answers: `may be invoked without any argument` NOT `must be invoked with exactly one argument`
...
<details>
  ...
  - Answers: `The `None` value can be assigned to variables` and `The `None` value can be compared with variables`
